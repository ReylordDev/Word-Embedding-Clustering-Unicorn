...
def outlier_detection(stereotypes, embeddings_normalized, OUTLIER_K = 5, OUTLIER_DETECTION_THRESHOLD = 1, SHOW_PLOTS = False):
  ...
  avg_neighbor_sim = np.mean(-np.partition(-S, OUTLIER_K + 1, axis = 1)[:, 1:OUTLIER_K+1], axis = 1)
  ...
  outlier_threshold = np.mean(avg_neighbor_sim) - OUTLIER_DETECTION_THRESHOLD * np.std(avg_neighbor_sim)

  outliers = avg_neighbor_sim < outlier_threshold

  if SHOW_PLOTS:
    plt.plot(-np.sort(-avg_neighbor_sim))
    plt.plot([0, len(avg_neighbor_sim)], [outlier_threshold, outlier_threshold], 'r--')
...
def find_number_of_clusters(stereotypes, embeddings_normalized, MAX_NUM_CLUSTERS, sample_weights = None, SHOW_PLOTS = False):
  ...
  if MAX_NUM_CLUSTERS < 50:
    ...
    Ks = list(range(2, 51)) + list(range(55, 101, 5)) + list(range(110, MAX_NUM_CLUSTERS, 10))
...
def cluster_and_merge(stereotypes, embeddings_normalized, K, sample_weights = None, MERGE_THRESHOLD = 1., SHOW_PLOTS = False):
  ...
  if MERGE_THRESHOLD is not None and MERGE_THRESHOLD < 1.:
    if SHOW_PLOTS:
      print('Merging similar clusters together until the cosine similarity of all cluster centers is below %g' % MERGE_THRESHOLD)
    # merge the closest clusters using Agglomorative Clustering
    # until everything is closer than the threshold
    meta_clustering = AgglomerativeClustering(n_clusters = None, distance_threshold = 1. - MERGE_THRESHOLD, linkage = 'complete', affinity = 'cosine')
    meta_clustering.fit(centers_normalized)

    # print which clusters got merged together
    if SHOW_PLOTS:
      for l in np.unique(meta_clustering.labels_):
        merged = np.where(meta_clustering.labels_ == l)[0]
        if len(merged) > 1:
          merged_exemplars = [exemplars[k] for k in merged]
          print('the following clusters got merged together: %s' % (', '.join(merged_exemplars)))
    ...
    for k in range(K_new):
      ...
      centers_new[k, :] = np.dot(sample_weights[in_cluster_k], embeddings_normalized[in_cluster_k, :]) / np.sum(sample_weights[in_cluster_k])
